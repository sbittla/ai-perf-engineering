...
====================================================================
LEARNING GOAL
    Write a complete, correct training loop from scratch.
    Every component you implement here will appear repeatedly throughout
    the performance engineering course — understanding it deeply means
    you can profile and optimise it instead of treating it as a black box.

    Components covered:
      Dataset + DataLoader  →  feeds batches to the model
      Forward pass          →  compute predictions
      Loss computation      →  measure error
      Backward pass         →  compute gradients
      Optimizer step        →  update weights
      Eval loop             →  measure generalisation
      Mixed precision (AMP) →  faster training with FP16

INSTRUCTIONS
    Fill in every TODO, run: python exercise_04_training_loop.py
"""
...
for epoch in range(N_EPOCHS):
    # ── TRAINING PHASE ────────────────────────────────────
    model.train()   # set train mode (dropout/BN active)
    train_losses = []
    ...
    # ── VALIDATION PHASE ──────────────────────────────────
    model.eval()   # set eval mode (dropout disabled)
    val_losses = []
...
if DEVICE != "cuda":
    ...
    # It multiplies the loss before backward (scales up), then divides after
...
# HINTS
# ─────────────────────────────────────────────────────────
# TODO 1:  return len(self.x)
# TODO 2:  return self.x[idx], self.y[idx]
# TODO 3:  DataLoader(train_ds, batch_size=32, shuffle=True, num_workers=0)
# TODO 4:  DataLoader(val_ds, batch_size=64, shuffle=False, num_workers=0)
# TODO 5:  nn.Sequential(nn.Linear(1,64), nn.ReLU(), nn.Linear(64,64), nn.ReLU(), nn.Linear(64,1))
# TODO 6:  model = model.to(DEVICE)
# TODO 7:  xb = xb.to(DEVICE); yb = yb.to(DEVICE)
# TODO 8:  optimizer.zero_grad(set_to_none=True)
# TODO 9:  pred = model(xb)
# TODO 10: loss = criterion(pred, yb)
# TODO 11: loss.backward()
# TODO 12: optimizer.step()
# TODO 13: with torch.no_grad():  (indent the for loop)
# TODO 14: with torch.autocast(device_type="cuda", dtype=torch.float16):
#               pred = model_amp(xb)
#               loss = criterion(pred, yb)
# TODO 15: scaler.scale(loss).backward()
# TODO 16: scaler.step(opt_amp); scaler.update()
# TODO 17: pred_y = model(test_x)
