...
LEARNING GOAL
    ...
    physically attached to CPU socket 0 is faster for socket 0 processes.
    Accessing socket 1's memory from socket 0 crosses the inter-socket
    interconnect (QPI/UPI) — 1.5–2.5× slower.

    On RTX 4060 workstations this matters when:
      • The GPU is on socket 0's PCIe lanes
      • DataLoader workers running on socket 1 read data and send to GPU
        Path: Socket1 RAM → QPI → Socket0 → PCIe → GPU VRAM  (slow!)
      • Fix: numactl --cpunodebind=0 python train.py  → workers stay local

    On single-socket systems (most desktop/laptops): NUMA nodes still
    exist but distance=10 for all nodes → no inter-socket penalty.
    This script still teaches the commands and principles.
