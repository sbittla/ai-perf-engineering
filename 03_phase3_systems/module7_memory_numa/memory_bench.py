...
============================================================================
PHASE: 3 (Module 7 — Memory & NUMA Optimization)

PURPOSE:
    Measure and compare memory bandwidth at every level of the hierarchy:
    CPU L1/L2/L3 cache, DRAM, and the PCIe Host-to-Device (H2D) transfer path.
    Then demonstrate NUMA effects by forcing cross-socket memory access.

WHAT YOU WILL MEASURE:
    1. CPU memory bandwidth at different working set sizes (cache vs DRAM)
    2. PCIe bandwidth: host (CPU RAM) → device (GPU VRAM)
    3. GPU DRAM bandwidth (device-to-device copy)
    4. NUMA local vs remote memory access latency
    5. Effect of pinned (page-locked) vs pageable memory on H2D bandwidth

WHY THIS MATTERS FOR AI WORKLOADS:
    - DataLoader: reading images from disk → CPU RAM → GPU
      Limited by disk → DRAM → PCIe bandwidth chain
    - LLM decode: reading KV cache every token → GPU DRAM bandwidth
      Decode is MEMORY-BANDWIDTH BOUND — this is the ceiling
    - Multi-GPU: gradients travel GPU→GPU via NVLink or PCIe
      All-reduce is bottlenecked by the inter-GPU bandwidth
