...
=======================================================================
LEARNING GOAL
    Understand HOW to create and MEASURE CPU cache misses, and why they
    matter for DataLoader and preprocessing performance.

    A cache MISS forces the CPU to wait 100–300 cycles for data from DRAM.
    A cache HIT serves data in 4–50 cycles from L1/L2/L3.

    For AI workloads cache misses appear in:
      • DataLoader workers reading sparse indices across large datasets
      • CPU augmentation touching many random image regions
      • Tokenizer vocabulary lookups on large vocab tables
      • NumPy / pandas preprocessing on large arrays

CACHE ARCHITECTURE (reminder)
    L1 : ~32 KB  per core, ~4 cycles
    L2 : ~256 KB per core, ~12 cycles
    L3 : ~8-32 MB shared,  ~40 cycles
    DRAM: GB range,         ~200 cycles

    "Working set" = total data your code touches in a tight loop.
    If working_set > L3, every access is a DRAM read → cache thrash.

HOW TO MEASURE CACHE MISSES
    # With perf (most accurate, needs kernel access):
    sudo perf stat -e cache-misses,cache-references,L1-dcache-misses \\
        python cache_miss_analysis.py

    # Without sudo (userspace counters):
    perf stat -e cycles,instructions python cache_miss_analysis.py
