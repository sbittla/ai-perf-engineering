...
=============================================================================
LEARNING GOAL
    Lock contention happens when multiple threads compete for the same
    mutex/lock.  The losing threads BLOCK (sleep in the kernel) waiting
    for the lock to be released.  This shows up as:
      • High %sy (system CPU) in vmstat
      • Many futex() syscalls in strace
      • "lock_contention" in perf report
      • Flat top of flamegraph labelled pthread_mutex_lock or futex

    For AI workloads this appears in:
      • Python GIL contention between DataLoader workers and main thread
      • PyTorch internal allocator locks under concurrent GPU streams
      • NCCL internal mutexes during all-reduce

WHAT THIS SCRIPT DOES
    Creates a shared counter accessed by N threads simultaneously,
    with and without contention, then shows you how to observe the
    difference with Linux tools.
