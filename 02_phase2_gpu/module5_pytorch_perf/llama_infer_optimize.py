...
=====================================================================================
LEARNING GOAL
    Apply each optimisation to a small GPT/Llama-style model step-by-step
    and record the tok/s improvement after each change.  At the end you
    have a before/after comparison table showing cumulative speedup.

OPTIMISATION LADDER (each step builds on the previous)
    Step 0  Baseline       FP32 eager, no compile
    Step 1  FP16           halve weight memory + tensor cores
    Step 2  BF16           same as FP16 but more numerically stable
    Step 3  torch.compile  kernel fusion, operator reordering
    Step 4  KV cache       avoid recomputing past keys/values
    Step 5  CUDA graphs     eliminate kernel launch overhead for fixed shapes
