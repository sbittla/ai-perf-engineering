...
=========================================================================
LEARNING GOAL
    Quantization reduces model precision from FP16 (2 bytes/weight) to
    INT8 (1 byte) or INT4 (0.5 bytes).  This cuts memory bandwidth and
    VRAM usage, at the cost of some accuracy.

    For MEMORY-BANDWIDTH-BOUND workloads (LLM decode), reducing weight
    size directly improves throughput:
        decode_throughput ≈ GPU_BW / bytes_per_weight / model_params

QUANTIZATION TYPES COVERED
    Dynamic INT8  : quantize weights at inference time, per-tensor scale
                    Simple, no calibration data needed, slight accuracy loss
    Static INT8   : calibrate scales offline on representative data
                    Better accuracy, requires calibration step
    BitsAndBytes  : 8-bit and 4-bit via LLM.int8() / nf4/fp4 formats
                    Used by HuggingFace bitsandbytes integration
    GPTQ          : post-training quantization optimizing weight rounding
                    Common in production (TheBloke models on HF Hub)
    AWQ           : activation-aware weight quantization
                    State-of-art accuracy at INT4, used by vLLM
