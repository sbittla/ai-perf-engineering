...
==============================================================================
LEARNING GOAL
    Understand the three floating-point precisions used in LLM training
    and inference, measure their real throughput difference, and learn
    when to use each.

FP32  (32-bit float)
    Range:  ±3.4×10³⁸   Precision: ~7 decimal digits
    Use:    master weights, optimizer states, loss scaling reference
    Speed:  baseline (1×)

FP16  (IEEE 16-bit float)
    Range:  ±65504      Precision: ~3 decimal digits
    Issue:  small gradients underflow to zero → training instability
    Fix:    gradient scaling (GradScaler) or BF16
    Speed:  ~2–4× faster than FP32 on tensor cores

BF16  (bfloat16, "brain float")
    Range:  ±3.4×10³⁸   Precision: ~2 decimal digits  (same range as FP32!)
    Use:    modern LLM training (stable, same range as FP32, no grad scaling)
    GPU:    Ampere+ (RTX 30xx, A100, H100) — NOT supported on Turing (RTX 20xx)
    Speed:  ~same as FP16

KEY RULE:
    Training:   BF16 (no grad scaling needed) or FP16 + GradScaler
    Inference:  FP16 (smaller memory, tensor cores, near-FP32 accuracy)
    Activations: BF16 preferred for stability
    Weights:    FP32 master copy + FP16 compute copy
