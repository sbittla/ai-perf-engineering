...
=================================================================
PHASE: 2 (GPU Profiling), 3 (Linux Systems Profiling)

PURPOSE:
    A complete, realistic training loop that is the TARGET for profiling
    with nsys, ncu, perf, strace, and flamegraphs. Every profiling command
    in the roadmap runs against this script:
    ...
    It's deliberately complete (data loading, forward, backward, logging)
    so that profiling it reveals realistic bottlenecks, not toy ones.

WHAT THIS TRAINS:
    Option A (--task lm):    Language model — TinyTransformer on random tokens
    Option B (--task image): Image classifier — SmallCNN/ResNet on random images

    Both options produce realistic GPU workloads for profiling.

WHAT TO OBSERVE AS YOU PROFILE:
    With nsys:
        - Are there gaps between CUDA kernels? (CPU bottleneck)
        - How much time is H2D transfer vs compute?
    With ncu:
        - What's the SM utilisation of your attention kernels?
        - Memory-bound or compute-bound?
    With perf:
        - Which C functions consume most CPU cycles?
    With strace:
        - How many syscalls happen per training step?
