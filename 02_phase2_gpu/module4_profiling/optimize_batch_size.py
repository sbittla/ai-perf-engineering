...
============================================================================
LEARNING GOAL
    Sweep batch sizes and measure throughput (samples/sec) and latency
    (ms/batch).  Find the knee where:
      • Throughput plateaus  → GPU memory-bandwidth saturated
      • Latency is still acceptable for your SLA

    This is a required step before any production deployment.
    The optimal batch size differs per model, GPU, and task.

KEY CONCEPTS
    Throughput  = samples / second  (maximize for offline inference)
    Latency     = ms / batch        (minimize for interactive use)
    These conflict: bigger batch → higher throughput, higher latency.

    GPU utilisation rises with batch until either:
      (a) Memory bandwidth saturates → throughput plateau
      (b) VRAM runs out              → OOM
