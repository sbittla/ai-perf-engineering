...
=========================================================================
LEARNING GOAL
    Occupancy = (active warps on SM) / (max warps an SM can hold).
    High occupancy lets the GPU hide memory latency by switching between
    warps while one waits for data.  Low occupancy leaves SMs idle.

    Three main occupancy limiters:
      1. Shared memory  – large allocation per block → fewer blocks per SM
      2. Registers      – many registers per thread  → fewer threads per SM
      3. Block size     – too small → not enough warps to fill SM

    This script measures the *observable effect* of low vs high occupancy
    using PyTorch ops that simulate different resource pressures.

HOW TO MEASURE ACTUAL OCCUPANCY
    ncu --metrics sm__warps_active.avg.pct_of_peak_sustained_active \\
        python occupancy_experiment.py

    nsys profile --stats=true python occupancy_experiment.py
