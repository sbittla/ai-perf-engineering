...
======================================================================
PHASE: 2 (Module 3 — CUDA Fundamentals, Module 4 — GPU Profiling)

PURPOSE:
    Hands-on experiments that teach GPU execution model concepts by
    measuring them directly. Each experiment isolates one concept:

    EXP 1 — Occupancy:        How full is the GPU? (active warps / max warps)
    EXP 2 — Memory Coalescing: Accessing memory in stride-1 vs stride-N
    EXP 3 — Warp Divergence:  if/else inside a warp → inactive threads
    EXP 4 — Tensor Cores:     FP16 matmul (tensor cores) vs FP32 (CUDA cores)
    EXP 5 — Memory Hierarchy: L1 vs L2 vs DRAM access latency pattern
    EXP 6 — Kernel Fusion:    One fused kernel vs two separate kernels

KEY CONCEPTS:

    WARP:
        The GPU executes 32 threads simultaneously in a "warp".
        All 32 threads in a warp execute the SAME instruction (SIMT).
        If threads branch differently (if/else), the warp must execute
        BOTH branches serially with inactive threads masked — this is
        warp divergence and halves (or worse) your effective throughput.

    OCCUPANCY:
        Ratio of active warps to maximum warps on a Streaming Multiprocessor (SM).
        High occupancy = GPU can hide memory latency by switching warps.
        Low occupancy (often due to large register/shared memory usage per thread)
        = GPU sits idle waiting for memory.

    MEMORY COALESCING:
        When 32 threads in a warp each load one float, the GPU coalesces
        these into one 128-byte memory transaction — IF addresses are
        contiguous (stride-1). Non-contiguous access (stride > 1) requires
        multiple transactions → lower effective bandwidth.
