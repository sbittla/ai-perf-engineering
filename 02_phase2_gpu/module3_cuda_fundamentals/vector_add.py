...
=============================================================
LEARNING GOAL
    Understand the GPU execution model by building the simplest possible
    GPU program: add two vectors element-wise.  Every concept that matters
    for LLM inference (warps, blocks, grids, occupancy, memory layout)
    appears here in its most stripped-down form.

GPU EXECUTION MODEL (read before running)
─────────────────────────────────────────
    Thread  : one lane of execution.  Each thread runs the same code but
              with a unique thread ID it uses to pick its data element.

    Warp    : 32 threads that execute in lock-step on one SM.
              If they diverge (if/else), the warp serialises — slow.

    Block   : a group of threads (up to 1024) that share fast L1/shared
              memory and can synchronise with __syncthreads().

    Grid    : the collection of all blocks launched for one kernel call.
              Blocks are assigned to SMs independently.

    Mapping to vector add:
        N elements → N threads, one per element
        Threads grouped into blocks of BLOCK_SIZE (e.g. 256)
        Number of blocks = ceil(N / BLOCK_SIZE)  ← the "grid size"

        Thread (block_idx, thread_idx) handles element:
