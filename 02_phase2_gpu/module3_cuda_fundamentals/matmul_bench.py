...
============================================================================
PHASE: 2 (Module 4 — GPU Profiling & Optimization)

PURPOSE:
    Benchmark matrix multiplication across sizes and precisions to:
    1. Find where YOUR GPU transitions from memory-bound to compute-bound
    2. Build an empirical roofline model using measured peak bandwidth + FLOPS
    3. Show how batch size, dtype, and tiling affect throughput
    4. Serve as the profiling target for ncu roofline analysis

THE ROOFLINE MODEL (key concept for GPU performance):
    Every kernel is limited by ONE of two ceilings:
    
    a) Memory Bandwidth Ceiling:
       If the kernel reads/writes more data than it computes,
       you'll hit memory bandwidth before hitting compute.
       Throughput ≤ Memory_Bandwidth × (FLOPS / bytes_accessed)
    
    b) Compute Ceiling:
       If the kernel does a lot of arithmetic per byte loaded,
       you'll hit peak FLOPs before hitting memory bandwidth.
       Throughput ≤ Peak_FLOPS

    The "arithmetic intensity" (AI) = FLOPs / bytes tells you which ceiling:
        AI < ridge_point → memory-bound  (increase reuse to improve)
        AI > ridge_point → compute-bound (only faster GPU helps)

    For matrix multiply M×K×N:
        FLOPs    = 2·M·K·N
        Bytes    = (M·K + K·N + M·N) × dtype_bytes
        AI       = 2MKN / ((MK + KN + MN) × bytes)
    
    Large square matrices → high AI → compute-bound → GPU fully utilised
    Tiny matrices (batch=1 decode step) → low AI → memory-bound
