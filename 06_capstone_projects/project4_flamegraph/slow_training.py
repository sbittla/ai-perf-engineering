...
==================================================================================
PURPOSE:
    A training script that deliberately contains the 5 most common Python-side
    performance mistakes. Each is clearly labelled so you can:
    
    1. Profile this script with py-spy/perf to see the bottlenecks as flamegraph
    2. Apply the fixes in fast_training.py
    3. Generate a differential flamegraph showing what was removed

THE 5 INTENTIONAL BOTTLENECKS:
    A. Heavy CPU augmentation in the data loop
       → CPU is busy doing work the GPU could do
    
    B. Synchronous .item() call every step
       → Inserts a CPU-GPU barrier on EVERY training step
    
    C. Blocking .to(device) without non_blocking=True
       → CPU waits for H2D transfer to finish before continuing
    
    D. File I/O logging inside the training loop
       → Disk write every step; GIL acquisition; context switches
    
    E. Python list accumulation with .append() inside loop
       → Memory allocation + garbage collection pressure

HOW TO PROFILE THIS SCRIPT:
    # Generate flamegraph while this runs:
    bash profile_flamegraph.sh slow
    
    # Quick bottleneck report (Python + CUDA combined):
    python -m torch.utils.bottleneck slow_training.py --steps 30
"""
