...
==============================================================
PURPOSE:
    The optimised version of slow_training.py. Each fix is annotated
    with WHY it helps, so this file teaches by example.

FIXES APPLIED:
    A. GPU augmentation via torchvision.transforms.v2
       → augmentation happens on GPU after H2D transfer
       → CPU workers only do cheap: load raw image, convert to tensor

    B. Remove .item() from inner loop
       → no GPU barrier on every step
       → accumulate on GPU, sync only every N steps or at logging time

    C. non_blocking=True on .to(device)
       → H2D transfer in background DMA; CPU continues immediately
       → model can start as soon as transfer completes (overlapped)

    D. Logging to in-memory buffer, flushed every N steps
       → eliminates per-step disk I/O
       → single flush every 20 steps instead of 20 flushes

    E. Pre-allocated numpy arrays instead of growing Python lists
       → zero memory reallocation; predictable memory usage

HOW TO PROFILE THE FIXED VERSION:
    bash profile_flamegraph.sh fast    # generates fast_profile.svg
    bash diff_flamegraph.sh            # shows what got removed
"""
