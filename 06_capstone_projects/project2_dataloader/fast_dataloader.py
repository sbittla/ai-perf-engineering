...
================================================================
PURPOSE:
    The fixed version of slow_dataloader.py. Apply every DataLoader
    optimisation and verify with the same Linux tools that the bottleneck
    is gone. This is the "after" state.

OPTIMISATIONS APPLIED:
    1. num_workers=8          — parallel background workers pre-load batches
                                while the GPU processes the previous batch
                                (pipeline overlap: CPU + GPU work simultaneously)

    2. pin_memory=True        — allocate CPU tensors in pinned (page-locked) memory
                                GPU can DMA directly from it — eliminates one copy
                                and lets transfers happen asynchronously

    3. non_blocking=True      — .to(device, non_blocking=True) returns immediately
                                the actual H2D transfer happens in a background stream
                                CPU can prepare the loss/optimiser step while data moves

    4. prefetch_factor=2      — each worker pre-loads N batches ahead
                                by the time GPU finishes batch N, batch N+1 is ready

    5. GPU augmentation       — move random crop/flip to GPU using torchvision v2
                                transforms.RandomResizedCrop → faster on GPU tensor
                                (eliminate the CPU bottleneck entirely)

    6. Remove .item() from loop — only sync for logging, not every step
                                 accumulate loss on GPU, sync every 10 steps

    7. NUMA binding (optional) — bind workers to NUMA node closest to GPU
                                 reduces cross-socket memory access latency

EXPECTED IMPROVEMENT:
    GPU utilisation: ~40% → ~90%+
    Step time: 3-5x faster
    GPU idle gaps: mostly eliminated
