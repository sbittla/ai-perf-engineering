...
============================================================================
PURPOSE:
    Deliberately create a GPU-starved training scenario by introducing
    realistic DataLoader inefficiencies. This is the "before" state you
    will diagnose with Linux tools and then fix in fast_dataloader.py.

BOTTLENECKS INTRODUCED (intentionally):
    1. num_workers=0         — single-threaded data loading (no parallelism)
                               GPU waits idle while CPU fetches the next batch
    2. No pin_memory          — data transferred through pageable memory
                               OS must copy data through intermediate buffers
                               before GPU DMA can access it (extra copy)
    3. Synchronous .item()   — forces CPU-GPU synchronisation every batch
                               GPU must finish current work before CPU continues
    4. CPU augmentation      — heavy image transforms on CPU while GPU waits
    5. No prefetching        — CPU only starts loading batch N+1 after GPU
                               finishes batch N (no pipeline overlap)

WHAT YOU SHOULD OBSERVE WHILE THIS RUNS:
    Terminal 2: watch -n 1 nvidia-smi
        → GPU utilisation should be LOW (30–60%) and fluctuating
        → GPU idle during DataLoader fetches

    Terminal 2: vmstat 1
        → 'wa' column (iowait) may be elevated
        → 'us' column (user CPU) peaks during augmentation

    Terminal 2: iostat -xz 1
        → If using slow disk, %util will be high on that device

    Terminal 2: nsys profile python slow_dataloader.py
        → Timeline shows regular "gaps" between CUDA kernel bursts
