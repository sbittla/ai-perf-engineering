...
=============================================================================
PURPOSE:
    Simulate a realistic multi-user LLM serving scenario by sending N
    concurrent requests to a vLLM server. Measure how throughput and latency
    degrade as concurrent load increases — this directly stresses the KV cache.

WHY CONCURRENCY STRESSES KV CACHE:
    Each request holds its own KV cache while generating tokens.
    With 20 concurrent requests each needing 2048 tokens of KV cache:
      KV cache size = num_layers × 2 × num_heads × head_dim × seq_len × dtype_bytes
      For Llama-2-7B FP16: ~32 layers × 2 × 32 heads × 128 dim × 2048 × 2 bytes ≈ 1GB per request
    
    At 20 concurrent requests: 20GB needed — exceeds RTX 4060 8GB!
    vLLM handles this with PagedAttention (eviction + recompute) but still
    shows measurable degradation you can observe and tune.

METRICS CAPTURED:
    - TTFT   : Time to First Token — how long the user waits before seeing output
    - TBT    : Time Between Tokens — per-token decode latency (streaming experience)
