...
======================================================================
PURPOSE:
    Run Llama-2-7B (or a smaller model for quick testing) using standard
    HuggingFace transformers generate(). Measure tokens/sec accurately
    using CUDA Events — the correct way to time GPU work.

    This is the BEFORE state. Every optimisation in the next scripts
    will be compared back to these numbers.

WHY CUDA EVENTS instead of time.time()?
    time.time() measures wall-clock time on the CPU.
    GPU operations are ASYNCHRONOUS — the CPU submits work to the GPU
    and moves on immediately. Calling time.time() after model() returns
    does NOT mean the GPU has finished.
    
    torch.cuda.Event(enable_timing=True) records a timestamp on the GPU
    timeline itself, giving accurate GPU execution time.
    torch.cuda.synchronize() forces the CPU to wait until the GPU truly
    finishes before we stop the clock.

WHAT TO OBSERVE:
    - Tokens/sec (the primary LLM inference metric)
