...
==========================================================================
PURPOSE:
    Compare inference performance across 4 modes:
      1. Eager (baseline — no compilation)
      2. torch.compile default
      3. torch.compile reduce-overhead  
      4. torch.compile max-autotune

    torch.compile works by capturing the computation graph, then using
    a backend compiler (TorchInductor with Triton) to generate optimised
    GPU kernels. Key optimisations:
      - Kernel fusion: combine multiple small ops into one GPU kernel
        (eliminates memory round-trips between ops)
      - Operator reordering: move cheap ops to overlap with memory transfers
      - Layout optimisation: change tensor memory layouts for better coalescing

COMPILATION MODES EXPLAINED:
    default       : Balance between compile time and runtime performance.
                    Good for development — fast to compile, decent speedup.
    
    reduce-overhead: Aggressively eliminate Python/kernel-launch overhead.
                    Uses CUDA graphs to replay kernel sequences without Python.
                    Best for fixed-shape inputs (e.g., fixed batch + seq_len).
    
    max-autotune  : Benchmark multiple kernel implementations and pick fastest.
                    Long compile time (minutes) but best runtime performance.
                    Use this for production deployment on fixed hardware.

IMPORTANT: WARM UP MULTIPLE TIMES
    torch.compile is lazy — compilation happens on the first call.
    Second call may trigger re-compilation for different input shapes.
    Always warm up 3+ times before measuring compiled model performance.
