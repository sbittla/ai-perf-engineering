...
=================================================================
LEARNING GOAL
    FullyShardedDataParallel (FSDP) shards model parameters, gradients,
    and optimizer states across GPUs.  Unlike DDP which replicates the
    full model on every GPU, FSDP lets you train models larger than one
    GPU's VRAM by splitting ownership.

    Three shard levels:
      FULL_SHARD     : shard params + grads + optimizer states (most memory savings)
      SHARD_GRAD_OP  : shard grads + optimizer only (DDP-like param replication)
      NO_SHARD       : same as DDP (for debugging/comparison)

    Memory savings:
      DDP  : each GPU holds 1× params + 1× grads + 1× optimizer = 3× memory
      FSDP : each GPU holds 1/N params + 1/N grads + 1/N optimizer ≈ 3/N memory

    Communication:
      DDP  : all-reduce gradients once per backward step
      FSDP : all-gather params before each layer (forward) + reduce-scatter after
