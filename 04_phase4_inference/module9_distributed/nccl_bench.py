...
======================================================================
PHASE: 4 (Module 9 — Distributed AI Systems), 5 (Module 11 — Benchmarking)

PURPOSE:
    Benchmark NCCL collective operations (all-reduce, broadcast, all-gather)
    at different tensor sizes to find:
    1. Peak all-reduce bandwidth between your GPUs
    2. The latency floor (minimum time regardless of size)
    3. The bus bandwidth utilised vs theoretical peak
    4. Whether your GPUs are NVLink or PCIe connected

    This directly informs:
    - Can tensor parallelism be profitable on this hardware?
    - What gradient size triggers communication bottleneck in DDP training?
    - How does NCCL overlap with compute?

COLLECTIVE OPERATIONS EXPLAINED:
    All-Reduce:   Every GPU sends its gradient tensor. NCCL sums all copies
                  and every GPU receives the global sum. Used in DDP training.
                  Bandwidth: O(2 × (N-1)/N × message_size)

    All-Gather:   Every GPU sends its local shard. Every GPU receives all shards
                  concatenated. Used in FSDP (each GPU gets full param for forward).
                  Bandwidth: O((N-1)/N × total_size)

    Broadcast:    One GPU sends, all receive. Used for parameter sync at startup.

    Reduce-Scatter: Complement of all-gather. Each GPU sends full tensor,
                  receives 1/N shard of the reduced result. Used in FSDP backward.
