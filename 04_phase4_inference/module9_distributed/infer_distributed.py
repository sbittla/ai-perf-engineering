...
==================================================================================
PHASE: 4 (Module 9 — Distributed AI Systems)

PURPOSE:
    Run inference across multiple GPUs using tensor parallelism.
    Each GPU holds a SLICE of each weight matrix. The attention heads
    and FFN dimensions are split across GPUs, then an all-reduce gathers
    the partial results after each layer.

TENSOR PARALLELISM EXPLAINED:
    For a linear layer Y = X @ W (W shape: [d_model, d_model]):
    
    With 2 GPUs (column-parallel split):
        GPU 0: W0 = W[:, :d_model//2]  → Y0 = X @ W0  (partial result)
        GPU 1: W1 = W[:, d_model//2:]  → Y1 = X @ W1  (partial result)
        All-reduce: Y = Y0 + Y1         (sum partial results)
    
    Benefits:
        - Each GPU processes HALF the FLOPs → lower latency
        - Each GPU stores HALF the weights → smaller per-GPU memory
    
    Cost:
        - All-reduce communication after each layer
        - Efficient only when NVLink bandwidth > communication overhead
        - On PCIe-connected GPUs: overhead may exceed the benefit

WHEN DOES TENSOR PARALLELISM HELP?
    ✓ Models too large for a single GPU (70B+ parameters)
    ✓ NVLink-connected GPUs (A100, H100 DGX nodes)
    ✓ Batch=1 decode where single-GPU is memory-bandwidth-limited
    ✗ Small models on single GPU: overhead exceeds benefit
    ✗ PCIe-only multi-GPU: communication too slow
