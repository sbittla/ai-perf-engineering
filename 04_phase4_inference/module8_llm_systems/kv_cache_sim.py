...
===========================================================================
PHASE: 4 (Module 8 — LLM Inference Systems)

PURPOSE:
    Before running a model, predict exactly how much GPU memory the KV cache
    will consume and whether it will fit. Then simulate the PagedAttention
    block allocation algorithm to understand why vLLM can serve more requests
    than naive HuggingFace generate().

KV CACHE FUNDAMENTALS:
    In autoregressive generation, each transformer layer computes keys (K)
    and values (V) for every input token. During decode, these are REUSED
    on every step rather than recomputed (this is the point of the cache).

    KV cache size per token per layer:
        2 (K and V) × n_heads × head_dim × dtype_bytes

    Total KV cache for a request of seq_len tokens:
        n_layers × 2 × n_heads × head_dim × seq_len × dtype_bytes

    Example — Llama-2-7B FP16:
        32 layers × 2 × 32 heads × 128 dim × seq_len × 2 bytes
        = 524,288 × seq_len bytes
        = 512KB × seq_len
        → 512 tokens: 256MB
        → 2048 tokens: 1GB
        → 4096 tokens: 2GB (RTX 4060 runs out quickly with multiple users!)

PAGED ATTENTION (vLLM):
    Standard attention pre-allocates a fixed max_seq_len block for each
    request — wasteful if the actual response is short.

    PagedAttention divides KV memory into fixed-size "pages" (blocks).
    Each page holds N tokens' worth of KV cache per layer.
    Pages are allocated on demand and freed when the request completes.
    This allows: variable-length sequences + more concurrent requests.
