...
PURPOSE:
    A standalone inference script that is the target for:
        nsys profile --trace=cuda,nvtx python infer.py
        ncu --set full python infer.py --tokens 10
        strace -c python infer.py
        py-spy record -o infer_flame.svg -- python infer.py

    It demonstrates the PREFILL and DECODE phases of autoregressive generation,
    exposing the different performance characteristics of each:

    PREFILL PHASE:
        Process all input tokens in one forward pass.
        Shape: (batch, seq_len, d_model) — many tokens at once.
        → Compute-bound (large matrix multiplies)
        → Higher arithmetic intensity → can use Tensor Cores efficiently

    DECODE PHASE:
        Generate one token per step.
        Shape: (batch, 1, d_model) — single token.
        → Memory-bandwidth bound (reads full KV cache every step)
        → Tiny arithmetic intensity → GPU underutilised for small batches

    This distinction is CRITICAL for understanding why LLM inference is hard
    to optimise, and why PagedAttention, speculative decoding, and batching
    each target different parts of the bottleneck.

WHAT TO OBSERVE:
    nsys timeline:
        → Long burst during prefill (many GPU kernels), then rhythmic
          short bursts per decode step. Gaps = CPU overhead per token.
    ncu on attention kernel:
        → Memory-bound during decode (dram__throughput near 100%)
        → Compute-bound during prefill (sm__throughput near 100%)
    strace -c:
        → See how many syscalls per token (file reads? mmap? unexpected?)
