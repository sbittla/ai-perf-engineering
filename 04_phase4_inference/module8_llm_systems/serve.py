...
PURPOSE:
    A simplified inference server that runs indefinitely, processing requests
    in a loop. This is the target for LIVE PROFILING techniques that attach
    to a running process instead of profiling startup.

    WHY A SEPARATE serve.py?
    Production inference servers run 24/7. You can't restart them to profile.
    Instead you use:
        nsys profile --delay=N --duration=M  → attach after N seconds
        py-spy record --pid <PID>            → attach to running process
        ncu --target-processes all           → profile running process

    serve.py simulates this: it runs indefinitely so you can attach profilers
    to a steady-state process, not one that's still initialising.

WHAT THIS SIMULATES:
    - Continuous batch generation (steady-state GPU utilisation)
    - Request queue processing (batch fills up, inference runs, result returned)
    - Memory utilisation growing then stabilising as KV cache fills
