...
====================================================================
LEARNING GOAL
    Speculative decoding uses a small DRAFT model to generate K candidate
    tokens cheaply, then verifies them all in one forward pass of the large
    TARGET model.  If all K tokens are accepted, you get K tokens for roughly
    the cost of 1 target-model step.

    WHY IT WORKS
        Standard decode: 1 target step → 1 token  (memory-bandwidth bound)
        Speculative:     1 draft run + 1 target verify → up to K tokens
        The target verify is one forward pass with seq+K tokens (still cheap
        if K is small, because the verify pass is compute-bound, not BW-bound).

    WHEN IT HELPS
        • Target model is large and memory-bandwidth bound on decode
        • Draft model is 5–20× smaller (e.g. 68M draft vs 7B target)
        • Output is "easy" for the draft (repetitive or formulaic text)
          → high acceptance rate α → big speedup

    SPEEDUP FORMULA
        Expected tokens per target step = (1 - α^(K+1)) / (1 - α)
        At α=0.8, K=5: E[tokens] ≈ 3.6  → 3.6× throughput gain
        At α=0.5, K=5: E[tokens] ≈ 1.97 → 2× gain
        At α=0.2, K=5: E[tokens] ≈ 1.25 → marginal gain

    IMPLEMENTATION DETAIL
        The draft generates K tokens autoregressively.
        The target runs ONE forward pass on [prompt + draft_tokens].
        Tokens are accepted/rejected using a rejection sampling rule
        that ensures the output distribution matches the target exactly.
