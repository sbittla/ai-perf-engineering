...
===============================================================================
LEARNING GOAL
    Understand WHY continuous batching (used by vLLM) dramatically improves
    GPU utilisation vs static batching (used by HuggingFace generate()).

STATIC BATCHING (HuggingFace generate)
    Requests grouped into a batch of size B.
    All requests MUST finish before the batch is returned.
    If request 0 needs 10 tokens and request 7 needs 200 tokens:
      Request 0 finishes at step 10 → GPU still pads zeros for 190 steps
      GPU computes wasted work on padding → low effective utilisation
    Also: new requests CANNOT join until the entire batch completes.

CONTINUOUS BATCHING (vLLM, TGI)
    Completed requests leave the batch IMMEDIATELY.
    New requests join the batch at the NEXT decode step.
    GPU always has a full batch of REAL work → no wasted padding compute.
    Effective batch size ≈ constant even with variable-length requests.

THROUGHPUT COMPARISON
    Static batch B=16, avg output 50 tokens, max output 200 tokens:
      GPU computes 200 steps for all 16 → 16×200=3200 token-steps
      But only 16×50=800 are real → 75% waste

    Continuous batching with 16 concurrency:
      GPU always has ~16 real requests → near-zero waste
      Effective 4× throughput improvement
