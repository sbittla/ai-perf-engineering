...
==============================================================================
PHASE: 2, 3, 4 (referenced by all training and inference scripts)

PURPOSE:
    Central file defining every model used across the capstone projects.
    Keeping models here avoids duplication and makes it easy to swap
    architectures without editing every script.

    Three model families are included:

    1. TinyTransformer  — a minimal GPT-style causal language model
       Small enough to train on a laptop CPU, yet exercises all the same
       code paths as a production LLM: attention, KV cache, positional
       embeddings, causal masking. Used for profiling experiments.

    2. SmallCNN / ResNetWrapper  — image classification models
       Used for DataLoader and training-loop experiments where we don't
       need language modelling complexity.

    3. get_model()  — factory function that returns any model by name
       All scripts call get_model(name, ...) so you can swap architectures
       from the command line without editing code.

LEARNING GOALS FOR THIS FILE:
    - Understand how transformer attention is implemented
    - See the KV cache interface (used in generate() methods)
    - Understand why model size → memory requirements
    - Practice reading PyTorch nn.Module structure
"""
