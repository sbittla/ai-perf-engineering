...
===================================================================================
PHASE: 5 (Module 10 — Workload Porting, Module 11 — Benchmarking & Characterisation)

PURPOSE:
    Demonstrate the impact of CPU affinity and NUMA node binding on DataLoader
    throughput. This is the script referenced in:

        taskset -c 0-7 python dataloader_worker.py
        numactl --cpunodebind=0 --membind=0 python dataloader_worker.py

    Shows three configurations and measures batch delivery rate for each:
    1. Default (OS assigns workers to any CPU core)
    2. taskset: all workers restricted to a specific set of cores
    3. NUMA-aware: workers bound to the NUMA node closest to the GPU

WHY CPU AFFINITY MATTERS FOR DATALOADER:
    DataLoader workers are separate OS processes. By default, the OS scheduler
    can move them to any CPU core, including cores on a different NUMA node
    from the GPU. Cross-NUMA memory access is 2× slower.

    In addition, if workers share CPU cores with the main training process,
    they compete for L3 cache → cache thrashing → slower data loading.

    taskset -c 0-7 means "only use cores 0 through 7". On a 16-core system
    with one GPU, a good strategy is:
        Cores 0-7:  DataLoader workers
        Cores 8-15: Training process

    numactl --cpunodebind=0 --membind=0 goes further:
        - All memory allocations go to NUMA node 0's RAM
        - Process only runs on node 0's CPUs
        - If the GPU is also on node 0's PCIe lane, transfers are fastest
