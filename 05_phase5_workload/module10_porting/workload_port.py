...
==============================================================================
LEARNING GOAL  (directly aligned with the JD)
    Run the SAME workload across different hardware/precision configurations
    and observe how the BOTTLENECK SHIFTS.  This is the core skill for a
    Workload Porting & Performance Engineer.

BOTTLENECK SHIFTS (why porting is non-trivial)
    CPU (single thread):       memory-bandwidth, DRAM latency, IPC
    CPU (multi-thread):        lock contention, NUMA, L3 cache pressure
    GPU FP32:                  compute-bound for large matmuls
    GPU FP16:                  tensor cores → compute ceiling rises 8×
                               → shifts bottleneck to memory bandwidth
    GPU INT8:                  weights ×2 smaller → BW bottleneck eases
                               → new bottleneck: dequantize overhead
    GPU FP16 + compile:        kernel fusion eliminates memory round-trips
                               → new bottleneck: residual unfused ops

    This "shifting bottleneck" pattern is WHY the job exists:
    You can't just recompile your CPU model and expect peak GPU performance.
    Each hardware target needs its own analysis and optimisation.
