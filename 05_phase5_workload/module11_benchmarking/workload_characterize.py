...
==================================================================================
PURPOSE (Module 10 + 11 combined)
    A complete workload characterization script that:
      1. Measures compute utilisation, memory bandwidth, and roofline position
      2. Sweeps batch sizes and sequence lengths to build throughput/latency curves
      3. Profiles each bottleneck phase (prefill vs decode)
      4. Generates a structured summary report you can save and compare

    This is the "before I submit a benchmark result" checklist used in:
      • MLPerf Inference submissions
      • Hardware comparison papers
      • Production capacity planning

WORKLOAD CHARACTERIZATION CHECKLIST
    □ Peak compute utilisation (SM%)
    □ Peak memory bandwidth utilisation (DRAM%)
    □ Roofline position: memory-bound or compute-bound?
    □ Throughput at max sustained load
    □ Latency P50/P95/P99 at each batch size
    □ GPU memory footprint (weights + KV cache)
    □ CPU utilisation during GPU work (should be < 20%)
    □ Thermal throttling check (sustained vs burst perf)
